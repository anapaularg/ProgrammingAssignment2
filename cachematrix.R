##The makeCacheMatrix and cacheSolve function work together to cache potentially time-consuming computations, in this case caculating the inverse of a matrix, since it will cache the value of the inverse so it doesn't have to be recomputed when needed. This is based on lexical scoping in R. We use the <<- operator to assign a value to an object in an environment that is different from the current environment.
...
## The function cacheSolve will return a matrix, which is the inverse of "x", by calling the new matrix returned by the makeCacheMatrix function, but it first checks if the inverse has already been calculated. If so, it just gets the inverse from the cache without calculating it again, but if the input is new, it calculates the inverse of the data and sets the inverse in the cache via the setinverse function. 
